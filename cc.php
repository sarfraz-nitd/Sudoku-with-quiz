<?php
	...
	session_start();
	$msg="";
	$startTime = strtotime("21:04:00");
	
	$endTime = strtotime("21:05:00") ;
  ...
  $p_id="";
	...
	if($old_player){
			...
			for($j=0;$j<9;$j++){
				...
				if($data==''){
					if($outp[$i][$j]=='')
						$q .= "<td><input class='input_field' name='inp_".$i.$j."' id='".$i.$j."' type='text' onkeyup='saveState()'  maxlength='1' style='' required/></td>";
					else
					 	$q .= "<td><input class='input_field' name='inp_".$i.$j."' id='".$i.$j."' type='text' onkeyup='saveState()' maxlength='1' value='".$outp[$i][$j]."' style='' required/></td>";
