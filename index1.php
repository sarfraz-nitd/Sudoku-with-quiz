  ...
  $p_id="";
	...
	if($old_player){
			...
			for($j=0;$j<9;$j++){
				...
				if($data==''){
					if($outp[$i][$j]=='')
						$q .= "<td><input name='inp_".$i.$j."' id='".$i.$j."' type='text' onkeyup='saveState()'  maxlength='1' style='width:30px; height:30px;font-size:1em; font-style:italic' required/></td>";
					else
					 	$q .= "<td><input name='inp_".$i.$j."' id='".$i.$j."' type='text' onkeyup='saveState()' maxlength='1' value='".$outp[$i][$j]."' style='width:30px; height:30px;font-size:1em; font-style:italic' required/></td>";
					 
				}else{
					//$q .= "<td>".$raw[$i][$j]."</td>";
					if($i ==  getFSecretRaw($j,$raw) || $i ==  getSSecretRaw($j,$raw)){
						$q .= "<td class='secret' ><input name='inp_".$i.$j."' id='".$i.$j."' type='text' value='".$outp[$i][$j]."' onclick='setVar(".$i.",".$j.")' data-popup-open='popup-1' maxlength='1' style='width:30px; height:30px;font-size:1em; color:red; font-style:italic' required readonly/></td>";
					}else
						$q .= "<td><input name='inp_".$i.$j."' id='".$i.$j."' type='text' value='".$outp[$i][$j]."' maxlength='1' style='width:30px; height:30px;font-size:1em; font-style:italic;color:blue; cursor:not-allowed' required readonly/></td>";
		...
		for($i=0;$i<9;$i++){
			...
			for($j=0;$j<9;$j++){
				...
				if($data==''){
					...
					//$q .= "<td>".$raw[$i][$j]."</td>";
					if($i ==  getFSecretRaw($j,$raw) || $i ==  getSSecretRaw($j,$raw)){
						$q .= "<td class='secret' ><input name='inp_".$i.$j."' id='".$i.$j."' type='text' value='?' onclick='setVar(".$i.",".$j.")' data-popup-open='popup-1' maxlength='1' style='width:30px; height:30px;font-size:1em; color:red; font-style:italic' required readonly/></td>";
					}else
						$q .= "<td><input name='inp_".$i.$j."' id='".$i.$j."' type='text' value='".$raw[$i][$j]."' maxlength='1' style='width:30px; height:30px;font-size:1em; font-style:italic;color:blue; cursor:not-allowed' required readonly/></td>";
...
<body>
	...
	<div id="ques-sec">
		...
		<li>For each question, if you give a wrong answer then the number will be revealed but you will get a penalty (equivalent to -- seconds) for each wrong answer.</li>
	...
	<div id="time-sec">
		...
		<center><p class="header-1">Player's Profile</p></center>
		<div id="player-info">
...
<div id="gameboard">
	<div style="margin-left: 23%;width: 11%;"><div id="saving" style=" background-color: yellow;opacity:0;">saving..</div></div>
	
	<table id="main-body" border="8 solid black" cellpadding="5" cellspacing="5">
		<?php echo $q;?>
		<input type="Submit" id="submit_game" name="submit_game"  Value="Check My Sudoku" style="width: 253px;height: 42px;background-color: gainsboro;font-style: italic;font-size: 1.3em;border-style: outset;border-radius: 18px;position: absolute;top: 80%;right: 8%;">
		<input type="Submit" id="finish_game" name="finish_game"  Value="Finish Game" style="width: 253px;height: 42px;background-color: gainsboro;font-style: italic;font-size: 1.3em;border-style: outset;border-radius: 18px;position: absolute;top: 81%;left:8%;">
	</table>
</div>
...
$(document).ready(function(){
    ...
    $("#finish_game").click(function(){
    	var retVal = confirm("Do You Want to end up?");
       if( retVal == true ){
		 window.onbeforeunload = null;
		 saveState();
		 logout();
