...
'50. Two train each 500500 metre long, are running in opposite directions on parallel tracks. If their speeds are 4545 km/hr and 3030 km/hr respectively, the time taken by the slower train to pass the driver of the faster one is',
...
$correct = array(
2,1,2,4,4,2,4,3,2,1,4,3,1,4,1,2,2,4,2,3,1,3,2,3,4,3,3,2,4,4,1,2,3,2,3,2,2,1,3,2,4,3,2,3,2,3,3,4,4,2,4,2,4,2,1,2,1,3,1,4,1,4,3,3,4,3,4,2,3,3,1,4,2,2,1,4,2,3,3,3,3,1,2,4,1,3,3,1,1,1,2,2,3,2,2,2,3,4,1,1,2,3,4,2,2,4,1,1,2,2,1,4,3,1,2,2,2,1,2,3,1,4,2,2,4,2,3,3);

for($i=0;$i<count($quesArray); $i++) echo $quesArray[$i]."<br>";
for($i=0;$i<count($ansArray);$i++) for($j=0;$j<count($ansArray[$i]); $j++) echo $ansArray[$i][$j]."<br>";
for($i=0;$i<count($correct);$i++) echo $correct[$i]."<br>";

echo "<br>".count($quesArray)."<br> ".count($ansArray)."<br> ".count($correct);
 ?>
